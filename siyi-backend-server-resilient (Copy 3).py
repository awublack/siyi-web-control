...
5. 全局 UDP Socket 复用，消除频繁创建的系统开销，降低发送延迟。
"""
...
# ── 核心优化：全局复用 UDP Socket，程序启动时创建一次，消除频繁创建的系统开销 ──
...
def _execute_send(yaw, pitch):
    ...
    try:
        # 核心优化：直接使用全局复用的 Socket 发送，不再每次创建
        g_udp_socket.sendto(pkt, (CAMERA_IP, UDP_PORT))
