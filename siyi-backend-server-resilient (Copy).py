#!/usr/bin/env python3
"""
SIYI A8 Mini 网页版后端服务 - 自愈稳定版
增加看门狗(Watchdog)机制，检测到画面卡死时自动重连 RTSP
"""
        ...
        while self.running:
            ...
            try:
                ...
                for packet in container.demux(stream):
                    ...
                    # 看门狗检测：如果超过 3 秒没有新包，强制重连
                    if time.time() - self.last_frame_time > 3.0:
                        logger.warning("⚠️ 检测到流超时，正在强制重连...")
                        break
                    ...
                    for frame in packet.decode():
                        ...
                        buf = io.BytesIO()
                        img.save(buf, format='JPEG', quality=40) # 降低质量以提高稳定性
