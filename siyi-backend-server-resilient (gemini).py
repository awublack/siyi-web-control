#!/usr/bin/env python3
"""
SIYI A8 Mini 网页版后端服务 - 自愈稳定修复版
修复了导致画面几秒后卡死的看门狗逻辑与 PyAV 探测参数错误
"""
...
class VideoCaptureThread(threading.Thread):
    def __init__(self, rtsp_url):
        ...
        self.last_frame_time = time.time()  # 初始化当前时间
        self.running = True
    ...
    def run(self):
        while self.running:
            ...
            try:
                ...
                # 修复参数：增大 probesize 确保能正确解析 H.264 头信息，添加原生网络超时机制
                container = av.open(self.rtsp_url, options={
                    'rtsp_transport': 'tcp',
                    'timeout': '3000000',         # 3秒网络读写超时 (微秒)
                    'stimeout': '3000000',        # 3秒连接超时 (微秒)
                    'probesize': '102400',        # 适度调大，防止 H.264 关键帧改变时解码器崩溃
                    'analyzeduration': '500000'   # 限制分析时长为 0.5 秒，保证首次连接速度
                })
                
                stream = container.streams.video[0]
                stream.thread_type = 'AUTO'       # 允许底层开启多线程解码
                ...
                for packet in container.demux(stream):
                    ...
                    for frame in packet.decode():
                        ...
                        buf = io.BytesIO()
                        img.save(buf, format='JPEG', quality=40) # 维持低质量以提高局域网传输稳定性
