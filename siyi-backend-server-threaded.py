#!/usr/bin/env python3
"""
SIYI A8 Mini 网页版后端服务 - 多线程稳定版
采用独立线程读取视频流，防止主进程在网络波动时卡死
"""
...
class VideoCaptureThread(threading.Thread):
    def __init__(self, rtsp_url):
        ...
        self.frame_queue = queue.Queue(maxsize=2) # 保持最新帧，低延迟
        self.running = True
    ...
    def run(self):
        while self.running:
            container = None
            try:
                logger.info(f"正在尝试连接 RTSP: {self.rtsp_url}")
                container = av.open(self.rtsp_url, options={
                    'rtsp_transport': 'tcp',
                    'stimeout': '3000000', # 3秒超时
                    'buffer_size': '1024000'
                })
...
def send_gimbal_angle_command(yaw_angle, pitch_angle, ip=CAMERA_IP, port=UDP_PORT):
    yaw, pitch = max(-36000, min(36000, int(yaw_angle))), max(-36000, min(36000, int(pitch_angle)))
    pkt = struct.pack('<H B H H B h h', 0x6655, 0x00, 0x0002, 0x0001, 0x01, yaw, pitch)
    pkt += struct.pack('<H', crc16(pkt))
    try:
        with socket.socket(socket.AF_INET, socket.SOCK_DGRAM) as s:
            s.sendto(pkt, (ip, port))
...
async def handle_gimbal_angle(request):
    try:
        data = await request.json()
        send_gimbal_angle_command(data.get('yaw', 0), data.get('pitch', 0), data.get('ip', CAMERA_IP))
        return web.json_response({'status': 'success'})
    except: return web.json_response({'status': 'error'}, status=500)
...
async def websocket_handler(request):
    ...
    try:
        while not ws.closed:
            try:
                # 从线程队列获取最新帧
                jpeg_data = video_thread.frame_queue.get(timeout=1.0)
