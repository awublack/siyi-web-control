...
def send_gimbal_command(yaw, pitch, ip=CAMERA_IP, port=UDP_PORT):
    ...
    try:
        ...
        sock.close()
        logger.info(f"发送云台命令：yaw={yaw}, pitch={pitch}")
        return True
...
def send_gimbal_angle_command(yaw_angle, pitch_angle, ip=CAMERA_IP, port=UDP_PORT):
    ...
    try:
        ...
        sock.sendto(pkt, (ip, port))
        sock.close()
        logger.info(f"发送角度命令：yaw={yaw_angle}, pitch={pitch_angle}")
        return True
    except Exception as e:
...
async def handle_gimbal_control(request):
    ...
    try:
        ...
        if success:
            return web.json_response({'status': 'success', 'yaw': yaw, 'pitch': pitch})
        else:
            return web.json_response({'status': 'error', 'message': 'Failed to send command'}, status=500)
    except Exception as e:
        logger.error(f"处理控制请求失败：{e}")
            ...
            try:
                for packet in container.demux(stream):
                    ...
                    for frame in packet.decode():
                        # 检查是否为视频帧
                        # 在某些版本的 PyAV 中，使用 isinstance 检查比检查 .type 更可靠
...
def main():
    """主函数"""
    print("=" * 60)
    print("SIYI A8 Mini 网页版后端服务 - 修复版")
    print("=" * 60)
    print(f"RTSP URL: {RTSP_URL}")
    print(f"云台控制：{CAMERA_IP}:{UDP_PORT}")
