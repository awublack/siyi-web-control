#!/usr/bin/env python3
"""
SIYI A8 Mini 网页版后端服务 - 人脸/物体跟踪 + 手动按键智能融合版 (极致丝滑版)
核心优化：
1. 取消跳帧：物体跟踪器每帧实时更新，消除位置延迟，确保 PID 输入连续。
2. 缩小死区：将物体跟踪死区缩小至 8/18（接近人脸的 5/12），实现细腻的中心微调。
3. 解除限速：允许 PID 满速输出，确保快速移动时的动态响应。
4. 防卡顿保底：WebSocket 推流严格限制 25FPS，平衡 CPU 占用与视觉流畅度。
"""
...
DEADZONE_IN = 5
DEADZONE_OUT = 12
# 物体跟踪死区 (缩小，向人脸看齐，实现丝滑微调)
OBJ_DEADZONE_IN = 8
OBJ_DEADZONE_OUT = 18

ALPHA_ERR = 0.4  # 稍微提高响应灵敏度
OUT_AVG_WIN = 3
CONTROL_DT = 0.025
...
def create_tracker():
    ...
    raise RuntimeError("Missing OpenCV contrib trackers")
                    ...
                    for frame in packet.decode():
                        ...
                        if is_manual:
                            ...
                            if self.tracking_mode == 'object' and self.tracker is not None:
                                # 核心改进：每帧都 update，不再跳帧，确保位置数据连续
                                success, bbox = self.tracker.update(bgr_img)
                                
                                if success:
                                    px, py, pw, ph = [int(v) for v in bbox]
                                    fx, fy = px + pw // 2, py + ph // 2
                                    dx, dy = fx - cx, fy - cy
                                    
                                    # 防飞熔断：如果误差超过画面 40%，说明跟踪器已经彻底飘了
                                    if abs(dx) > VIDEO_W * 0.4 or abs(dy) > VIDEO_H * 0.4:
                                        ...
                                        if now_t - self.last_control_t >= CONTROL_DT:
                                            # 核心改进：使用缩小后的死区 (8/18)，实现细腻的中心微调
                                            if abs(dx) <= OBJ_DEADZONE_IN: yaw_cmd = 0
                                            elif abs(dx) > OBJ_DEADZONE_OUT: yaw_cmd = self.pid_y.update(dx, has_target=True)
                                            else:
                                                ratio = (abs(dx) - OBJ_DEADZONE_IN) / (OBJ_DEADZONE_OUT - OBJ_DEADZONE_IN + 1e-5)
                                                yaw_cmd = int(self.pid_y.update(dx, has_target=True) * ratio)
                                                
                                            if abs(dy) <= OBJ_DEADZONE_IN: pitch_cmd = 0
                                            elif abs(dy) > OBJ_DEADZONE_OUT: pitch_cmd = self.pid_p.update(-dy, has_target=True)
                                            else:
                                                ratio = (abs(dy) - OBJ_DEADZONE_IN) / (OBJ_DEADZONE_OUT - OBJ_DEADZONE_IN + 1e-5)
                                                pitch_cmd = int(self.pid_p.update(-dy, has_target=True) * ratio)
                                            
                                            # 核心改进：解除速度限制，允许 PID 满速输出，保证动态响应
                        ...
                        # 压缩质量稍微提高一点到 60，保证画面清晰度，配合 25FPS 限流依然不会卡
